...
- WhatsApp prerequisite: On dev tenant the WhatsApp UI checkbox is disabled even with feature flag `whatsapp-self-sign-in-toggle: on` — likely a tenant-level business-account config. The MCP tool should still accept `enableWhatsappLogin=true` and let the backend reject if the tenant isn't provisioned (don't gate client-side).
- Related issue: #67
