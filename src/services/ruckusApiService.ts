...
export async function moveApToGroup(
  ...
  console.log(`[RUCKUS] Moving AP ${serialNumber} to group ${apGroupId} in venue ${venueId}`);
  
  // Get token with proper parameters
  const token = await getRuckusJwtToken(
    ...
    region || process.env.RUCKUS_REGION
  );
  
  // Build API URL based on region
  const apiHost = region && region.trim() !== ''
    ? `https://api.${region}.ruckus.cloud`
    : 'https://api.ruckus.cloud';
  const url = `${apiHost}/api/v1/venues/${venueId}/apGroups/${apGroupId}/aps/${serialNumber}`;
  ...
  try {
    const response = await axios.put(url, {}, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
        'x-ruckus-target-resource': 'client'
      }
    });

    const moveResponse = response.data;
    console.log('[RUCKUS] Move AP to group response:', moveResponse);

    // Check if we have a requestId for async operation
    const activityId = moveResponse.requestId;
    if (!activityId) {
      console.log('[RUCKUS] No requestId in response, operation may be synchronous');
      return {
        ...moveResponse,
        status: 'completed',
        message: 'AP moved to group successfully'
      };
    }

    console.log(`[RUCKUS] Got activity ID: ${activityId}, polling for completion...`);
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        console.log(`[RUCKUS] Activity status: ${activityDetails.state}`);

        // Check if operation is complete
        if (activityDetails.state === 'COMPLETE' || activityDetails.state === 'SUCCESS') {
          return {
            ...moveResponse,
            status: 'completed',
            message: 'AP moved to group successfully',
        ...
        if (activityDetails.state === 'FAILED' || activityDetails.state === 'ERROR') {
          return {
            ...moveResponse,
            status: 'failed',
        ...
        // If still in progress, increment retry count and continue
        retryCount++;
        console.log(`[RUCKUS] Move operation in progress, attempt ${retryCount}/${maxRetries}`);
        ...
        if (retryCount >= maxRetries) {
          return {
            ...moveResponse,
            status: 'timeout',
            message: 'Move AP to group status unknown - polling timeout',
            error: 'Failed to get activity status after maximum retries'
          };
        }
    ...
    console.error('[RUCKUS] Error moving AP to group:', error);
    
    // Check for specific error statuses
    if (error.response?.status === 404) {
      return {
        status: 'error',
        message: 'AP, AP group, or venue not found',
    ...
    if (error.response?.status === 409) {
      return {
        status: 'error',
        message: 'AP is already in this AP group',
        error: 'Conflict - AP already in group'
      };
    }
...
export async function getApExternalAntennaSettings(
  token: string,
  venueId: string,
  serialNumber: string,
  region: string = ''
  ...
  const apiHost = region && region.trim() !== ''
    ? `https://api.${region}.ruckus.cloud`
    : 'https://api.ruckus.cloud';
  const url = `${apiHost}/api/v1/venues/${venueId}/aps/${serialNumber}/externalAntennaSettings`;
