...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      {
        name: "get_venue_settings",
        description:
          "Get a venue's AP-level settings in one call. REQUIRED: venueId (use get_ruckus_venues to get venue ID). OPTIONAL: categories (array; default = all) — any of: 'radio' (per-radio settings), 'apModelBandMode' (AP-model band/mode), 'externalAntenna' (AP-model external antenna), 'antennaType' (AP-model antenna type). Returns a merged object keyed by the requested categories. NOTE: for WLAN-at-venue settings use get_venue_wifi_network_settings instead.",
        inputSchema: {
          type: "object",
          properties: {
            venueId: { type: "string", description: "ID of the venue (use get_ruckus_venues to get venue ID)" },
            categories: { type: "array", items: { type: "string", enum: ["radio", "apModelBandMode", "externalAntenna", "antennaType"] }, description: "Settings categories to return (default: all four)" },
          },
          required: ["venueId"],
        },
      },
      {
        name: "get_ap_group_settings",
        description:
          "Get an AP group's settings in one call. REQUIRED: venueId (use get_ruckus_venues to get venue ID) + apGroupId (use get_ruckus_ap_groups to get AP group ID). OPTIONAL: categories (array; default = all) — any of: 'radio', 'apModelBandMode', 'externalAntenna', 'antennaType', 'clientAdmissionControl'. Returns a merged object keyed by the requested categories.",
        inputSchema: {
          type: "object",
          properties: {
            venueId: { type: "string", description: "ID of the venue (use get_ruckus_venues to get venue ID)" },
            apGroupId: { type: "string", description: "ID of the AP group (use get_ruckus_ap_groups to get AP group ID)" },
            categories: { type: "array", items: { type: "string", enum: ["radio", "apModelBandMode", "externalAntenna", "antennaType", "clientAdmissionControl"] }, description: "Settings categories to return (default: all five)" },
          },
          required: ["venueId", "apGroupId"],
        },
      },
      {
        name: "get_ap_settings",
        description:
          "Get an access point's settings in one call. REQUIRED: venueId (use get_ruckus_venues to get venue ID) + apSerialNumber (use get_ruckus_aps to get AP serial number). OPTIONAL: categories (array; default = all) — any of: 'radio', 'clientAdmissionControl'. Returns a merged object keyed by the requested categories.",
        inputSchema: {
          type: "object",
          properties: {
            venueId: { type: "string", description: "ID of the venue (use get_ruckus_venues to get venue ID)" },
            apSerialNumber: { type: "string", description: "Serial number of the AP (use get_ruckus_aps to get AP serial number)" },
            categories: { type: "array", items: { type: "string", enum: ["radio", "clientAdmissionControl"] }, description: "Settings categories to return (default: both)" },
          },
          required: ["venueId", "apSerialNumber"],
        },
      },
      ...
      },
      {
        ...
        description:
          "Get detailed information for a specific SAML IdP profile (name, stored IdP metadata, metadataUrl, attributeMappings). REQUIRED: profileId (use query_saml_idp_profiles to get profile ID). OPTIONAL: set includeServiceProviderMetadata=true to also return, under serviceProviderMetadata, the SP-side metadata XML RUCKUS One generates (SP entityID like https://<your-ruckus-one-host>/saml/{profileId} + AssertionConsumerService URL) — the values you register on the external IdP (e.g. a SAML client's Client ID + Master SAML Processing URL).",
        inputSchema: {
          type: "object",
          properties: {
            profileId: {
            ...
            includeServiceProviderMetadata: {
              type: "boolean",
              description:
                "When true, also fetch and include the SP-side SAML metadata XML (entityID + ACS) under serviceProviderMetadata. Default false.",
            },
          },
          required: ["profileId"],
        },
      ...
      },
      {
        ...
        description:
          "Create a portal service profile by passing a single profileConfig object (full config). REQUIRED: profileConfig with name (the profile name) + content (the portal content object — styling/text/display settings). FOR TERMS & CONDITIONS, set them inside content: content.termsConditionConfig (RICH DOC — a Tiptap doc JSON, use build_terms_condition_config) OR content.termsConditionUrl (LINK TO URL) — mutually exclusive (server enforces GUEST-422xxx). content.componentDisplay.termsConditions=true shows the T&C checkbox. FOR MARKETING COMMUNICATION (guest marketing opt-in), set inside content: content.componentDisplay.marketingCommunication=true enables the component; content.guestForm.fields.{username,fullName,email,phone}.{enabled,required} (booleans) pick which contact fields the form collects — at least email or phone MUST be enabled (server enforces GUEST-422049); content.guestForm.marketingCommunication.content is the consent text as a Tiptap doc (use build_terms_condition_config to build it — same doc shape as T&C). Symmetric with update_portal_service_profile (which takes the same object as a partial). Use query_portal_service_profiles to verify creation.",
        inputSchema: {
          type: "object",
          properties: {
            profileConfig: {
              type: "object",
              description:
                "Full portal service profile configuration. Keys: name (profile name), content (portal content object). For T&C, set content.termsConditionConfig (rich-doc Tiptap JSON, use build_terms_condition_config) OR content.termsConditionUrl (URL) — mutually exclusive (GUEST-422xxx); content.componentDisplay.termsConditions toggles the checkbox. For marketing opt-in, set content.componentDisplay.marketingCommunication=true, content.guestForm.fields.{username,fullName,email,phone}.{enabled,required} (at least email or phone enabled — GUEST-422049), and content.guestForm.marketingCommunication.content (Tiptap consent doc, use build_terms_condition_config).",
            },
            maxRetries: {
              type: "number",
              description: "Maximum number of retry attempts (default: 20)",
        ...
        inputSchema: {
          ...
          properties: {
            ...
            profileConfig: {
              type: "object",
              description:
                "Partial portal service profile configuration — only the fields to change. Merged onto the current config (retrieve-then-merge). Keys: name (profile name), content (portal content object). For T&C, set content.termsConditionConfig (rich doc) OR content.termsConditionUrl (URL) and null the other when switching modes (they are mutually exclusive, GUEST-422xxx); content.componentDisplay.termsConditions toggles the checkbox. For marketing opt-in: content.componentDisplay.marketingCommunication, content.guestForm.fields.{username,fullName,email,phone}.{enabled,required} (deep-merged; at least email or phone enabled — GUEST-422049), content.guestForm.marketingCommunication.content (Tiptap consent doc); to disable, set marketingCommunication=false and null content.guestForm.marketingCommunication.",
            },
            maxRetries: {
              type: "number",
              description: "Maximum number of retry attempts (default: 20)",
        ...
        inputSchema: {
          ...
          properties: {
            ...
            portalServiceProfileId: {
              type: "string",
              description:
                "Portal service profile ID (REQUIRED for these captive-portal sub-types: guestPass, clickThrough, selfSignIn, hostApproval, cloudpath, directory, saml, workflow). NOT used by type=wispr, which renders an external 3rd-party portal via wisprConfig.captivePortalUrl instead. Use query_portal_service_profiles to get available IDs. Portal service profiles are portal-type-agnostic — the same profile can back any captive-portal sub-type; the WLAN's type field is what drives the captive-portal rendering.",
            },
            directoryServerProfileId: {
              type: "string",
              description:
                "Directory server profile ID (REQUIRED for type=directory — binds the Active Directory/LDAP server end users authenticate against). Use query_directory_server_profiles or create_directory_server_profile to get the ID.",
            },
            samlIdpProfileId: {
              type: "string",
              description:
            ...
            wisprConfig: {
              type: "object",
              description:
                "WISPr (3rd-party captive portal) external-provider config (REQUIRED for type=wispr). The AAA server is supplied separately via radiusServiceProfileId (authentication) + accountingRadiusServiceProfileId (accounting), reusing the enterprise association path.",
              properties: {
                captivePortalUrl: {
                  type: "string",
                  description:
                    "REQUIRED. The 3rd-party captive portal's login URL end users are redirected to (copy from your WISPr vendor's configuration).",
                },
                providerName: {
                  type: "string",
                  description:
                    "Display name for the external provider (default 'Custom Provider'). Sent as a custom provider — the named built-in providers (Aislelabs, Purple, etc.) are not exposed.",
                },
                redirectUrl: {
                  type: "string",
                  description:
                    "Optional URL to redirect users to after successful authentication.",
                },
                integrationKey: {
                  type: "string",
                  description:
                    "Optional 16-char integration key shared with the external portal. Auto-generated if omitted (mirrors the admin GUI).",
                },
              },
            ...
            radiusServiceProfileId: {
              type: "string",
              description:
                "RADIUS authentication service profile ID (REQUIRED for type=enterprise AND type=wispr — for WISPr this is the 3rd-party AAA authentication server; use query_radius_server_profiles to get ID of profile with type=AUTHENTICATION)",
            },
      ...
      },
      {
        ...
        description:
          "Build a correctly-shaped Tiptap doc JSON for a portal rich-doc field. PURE BUILDER — does NOT call the RUCKUS API. Pass the returned JSON as the Terms & Conditions 'termsConditionConfig' field OR the marketing opt-in 'content.guestForm.marketingCommunication.content' field of create_portal_service_profile / update_portal_service_profile (both use the same Tiptap doc shape). REQUIRED: mode. MODE REFERENCE: PLAIN (one paragraph, one text run). HYPERLINK (one paragraph with multiple text segments; segments that include href become Tiptap link marks). MULTILINE (one paragraph with text lines joined by hardBreak nodes). Server validates depth ≤10, ≤700 paragraphs, ≤100 text nodes/paragraph, ≤60k chars; only http/https hrefs are accepted.",
        inputSchema: {
          type: "object",
          properties: {
            mode: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "get_venue_settings": {
      try {
        const { venueId, categories } = request.params.arguments as { venueId: string; categories?: string[]; };
        const token = await tokenService.getValidToken();
        const result = await getVenueSettings(token, venueId, categories as any, process.env.RUCKUS_REGION);
        return toolResult(result);
      } catch (error: any) {
        console.error("[MCP] Error getting venue settings:", error);
        let errorMessage = `Error getting venue settings: ${error}`;
        if (error.response) { errorMessage += `\nHTTP Status: ${error.response.status}`; errorMessage += `\nResponse Data: ${JSON.stringify(error.response.data, null, 2)}`; errorMessage += `\nResponse Headers: ${JSON.stringify(error.response.headers, null, 2)}`; } else if (error.request) { errorMessage += `\nNo response received: ${error.request}`; }
        return { content: [{ type: "text", text: errorMessage }], isError: true };
      }
    }
    case "get_ap_group_settings": {
      try {
        const { venueId, apGroupId, categories } = request.params.arguments as { venueId: string; apGroupId: string; categories?: string[]; };
        const token = await tokenService.getValidToken();
        const result = await getApGroupSettings(token, venueId, apGroupId, categories as any, process.env.RUCKUS_REGION);
        return toolResult(result);
      } catch (error: any) {
        console.error("[MCP] Error getting AP group settings:", error);
        let errorMessage = `Error getting AP group settings: ${error}`;
        if (error.response) { errorMessage += `\nHTTP Status: ${error.response.status}`; errorMessage += `\nResponse Data: ${JSON.stringify(error.response.data, null, 2)}`; errorMessage += `\nResponse Headers: ${JSON.stringify(error.response.headers, null, 2)}`; } else if (error.request) { errorMessage += `\nNo response received: ${error.request}`; }
        return { content: [{ type: "text", text: errorMessage }], isError: true };
      }
    }
    case "get_ap_settings": {
      try {
        const { venueId, apSerialNumber, categories } = request.params.arguments as { venueId: string; apSerialNumber: string; categories?: string[]; };
        const token = await tokenService.getValidToken();
        const result = await getApSettings(token, venueId, apSerialNumber, categories as any, process.env.RUCKUS_REGION);
        return toolResult(result);
      } catch (error: any) {
        console.error("[MCP] Error getting AP settings:", error);
        let errorMessage = `Error getting AP settings: ${error}`;
        if (error.response) { errorMessage += `\nHTTP Status: ${error.response.status}`; errorMessage += `\nResponse Data: ${JSON.stringify(error.response.data, null, 2)}`; errorMessage += `\nResponse Headers: ${JSON.stringify(error.response.headers, null, 2)}`; } else if (error.request) { errorMessage += `\nNo response received: ${error.request}`; }
        return { content: [{ type: "text", text: errorMessage }], isError: true };
      }
    }
    ...
    case "get_saml_idp_profile": {
      try {
        const { profileId, includeServiceProviderMetadata = false } =
          request.params.arguments as {
            profileId: string;
            includeServiceProviderMetadata?: boolean;
          };
