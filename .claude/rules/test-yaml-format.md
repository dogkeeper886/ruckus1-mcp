...
steps:
  - name: Step description
    ...
    expectError: true              # Optional: this step is EXPECTED to fail — the
                                   # judge inverts the exit-code check (passes on
                                   # non-zero, fails on exit 0). For negative tests
                                   # that assert a tool surfaces an error (#107).
